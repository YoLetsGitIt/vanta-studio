...
export async function updateStudioProfile(name, address, widgetBgColor, widgetAccentColor, walkinCutPercent, personalCutPercent, aftercareInstructions, timezone, latitude, longitude, paymentRecordingRequirement, rescheduleWindowHours, widgetConsentTemplateId) {
  const headers = { ...(await authHeaders()), 'Content-Type': 'application/json' };
...
export async function sendSelectionLink(bookingId, expiresHours = 168, depositRequired = false, depositAmount = null, durationMinutes = null, estimatedQuote = null) {
  const headers = { ...(await authHeaders()), 'Content-Type': 'application/json' };
