...
export async function updateStudioProfile(name, address, widgetBgColor, widgetAccentColor, walkinCutPercent, personalCutPercent, aftercareInstructions, timezone, latitude, longitude, paymentRecordingRequirement, rescheduleWindowHours) {
  const headers = { ...(await authHeaders()), 'Content-Type': 'application/json' };
...
export async function generateConsentLink(email) {
  const headers = { ...(await authHeaders()), 'Content-Type': 'application/json' };
