...
export async function updateStudioProfile(name, address, widgetBgColor, widgetAccentColor, walkinCutPercent, personalCutPercent, aftercareInstructions) {
  const headers = { ...(await authHeaders()), 'Content-Type': 'application/json' };
...
export async function sendSelectionLink(bookingId, expiresHours = 168, depositRequired = false, depositAmount = null) {
  const headers = { ...(await authHeaders()), 'Content-Type': 'application/json' };
