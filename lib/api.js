...
export async function getStripeStatus() {
  const headers = await authHeaders();
  const res = await fetch(`${BACKEND()}/stripe/connect/status`, { headers });
...
export async function startStripeOnboarding(returnTo) {
  const headers = { ...(await authHeaders()), 'Content-Type': 'application/json' };
  const res = await fetch(`${BACKEND()}/stripe/connect/onboard`, {
...
export async function disconnectStripe() {
  const headers = await authHeaders();
  const res = await fetch(`${BACKEND()}/stripe/connect/disconnect`, { method: 'DELETE', headers });
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
